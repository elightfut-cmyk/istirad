    ...
    >
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 sticky top-4">
            ...
            <div className="mb-6">
              <h4 className="font-bold text-gray-700 dark:text-gray-200 mb-3 text-sm">الفئة</h4>
              <div className="space-y-2">
                {categories.map((cat) => (
                  <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedCategory === cat ? 'bg-[#4f46e5] border-[#4f46e5]' : 'border-gray-300 dark:border-gray-600 group-hover:border-[#4f46e5]'}`}>
                      {selectedCategory === cat && <div className="w-2.5 h-2.5 bg-white dark:bg-gray-800 rounded-sm"></div>}
                    </div>
                    <input
                      type="radio"
                      name="category"
                    ...
                    />
                    <span className={`text-sm ${selectedCategory === cat ? 'font-bold text-[#4f46e5]' : 'text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:text-white'}`}>{cat}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-3">
                <h4 className="font-bold text-gray-700 dark:text-gray-200 text-sm">أقصى سعر</h4>
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="block w-full pr-12 pl-4 py-3 border border-transparent bg-gray-50 dark:bg-gray-900 rounded-xl focus:bg-white dark:bg-gray-800 focus:ring-2 focus:ring-[#4f46e5] focus:border-transparent transition-all sm:text-sm"
              />
            </div>
          </div>

          {loading ? (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">جاري تحميل المنتجات...</div>
          ) : (
            <>
              {/* Products Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProducts.map((product) => (
                  <div key={product.id} className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700 group flex flex-col transition-all hover:shadow-lg hover:-translate-y-1">
                    <div className="h-48 bg-gray-50 dark:bg-gray-900 relative overflow-hidden flex items-center justify-center p-2">
                      <button 
                        onClick={(e) => { e.stopPropagation(); toggleWishlist(product.id); }}
                        className="absolute top-3 left-3 z-10 p-2 bg-white dark:bg-gray-800/80 backdrop-blur-sm rounded-full shadow-sm hover:bg-white dark:bg-gray-800 transition-colors"
              ...
              {filteredProducts.length === 0 && (
                <div className="bg-white dark:bg-gray-800 p-12 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col items-center justify-center text-center">
                  ...
                  <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-2">لم يتم العثور على منتجات</h3>
                  <p className="text-gray-500 dark:text-gray-400 max-w-md">عذراً، لا توجد منتجات تطابق معايير البحث والفلترة الخاصة بك.</p>
                  <button
                    onClick={() => { setSearchQuery(''); setSelectedCategory('الكل'); setMaxPrice(5000); }}
                    className="mt-6 px-6 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 font-medium rounded-xl hover:bg-gray-200 transition-colors"
                  >
                    إعادة ضبط الفلاتر
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md p-6 relative">
            ...
            <div className="mb-6 bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border border-gray-100 dark:border-gray-700">
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                {orderingProduct.discount_price > 0 && orderingProduct.discount_price < orderingProduct.price ? (
                  ...
                  <span className="font-bold text-gray-800 dark:text-gray-100">{formatCurrency(orderingProduct.price * 1.10)}</span>
                )}
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">المورد: <span className="font-bold text-gray-800 dark:text-gray-100">{orderingProduct.supplier?.name}</span></p>
            ...
            <div className="mb-6">
              ...
              {(orderingProduct.stock || orderingProduct.moq) && quantity > (orderingProduct.stock || orderingProduct.moq) && (
                <p className="text-red-500 text-xs mt-2">الكمية المطلوبة أكبر من المخزون المتوفر ({orderingProduct.stock || orderingProduct.moq} وحدة).</p>
              )}
            </div>
            ...
            <button 
              onClick={submitOrder}
              disabled={submittingOrder || quantity < 3 || ((orderingProduct.stock || orderingProduct.moq) && quantity > (orderingProduct.stock || orderingProduct.moq))}
