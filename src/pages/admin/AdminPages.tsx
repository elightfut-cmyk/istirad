...
import { LayoutDashboard, Users, ShoppingBag, MessageSquare, Ticket, FileText, HelpCircle, AlertTriangle, Plus, Edit2, Trash2, Check, X , Package , Lightbulb , Search } from 'lucide-react';
    ...
    >
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">قائمة الصفحات</h2>
          <button
            ...
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                  <th className="p-4 font-bold text-gray-700 dark:text-gray-200">العنوان</th>
                  <th className="p-4 font-bold text-gray-700 dark:text-gray-200">الرابط (Slug)</th>
                  <th className="p-4 font-bold text-gray-700 dark:text-gray-200">الحالة</th>
                  <th className="p-4 font-bold text-gray-700 dark:text-gray-200">الإجراءات</th>
                </tr>
              </thead>
              <tbody>
                {pages.map((page) => (
                  <tr key={page.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:bg-gray-900/50">
                    <td className="p-4 font-medium text-gray-900 dark:text-white">{page.title}</td>
                    <td className="p-4 text-gray-500 dark:text-gray-400" dir="ltr">{page.slug}</td>
                    <td className="p-4">
                      <button
                        onClick={() => handleToggleActive(page.id, page.is_active)}
                        className={`px-3 py-1 rounded-full text-xs font-bold ${
                          page.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'
                        }`}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1">العنوان</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1">الرابط (Slug)</label>
                <input
                  type="text"
                  value={formData.slug}
                  ...
                  dir="ltr"
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">يجب أن يكون باللغة الإنجليزية وبدون مسافات.</p>
              </div>
              
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1">المحتوى (HTML)</label>
                <textarea
              ...
              <div className="flex items-center gap-2 mt-4">
                ...
                />
                <label htmlFor="isActive" className="text-sm font-bold text-gray-700 dark:text-gray-200">تفعيل الصفحة (تظهر في الفوتر)</label>
              </div>
            </div>
