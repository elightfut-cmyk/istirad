...
import { LayoutDashboard, Store, Package, CreditCard, Heart, Users, AlertTriangle , Search } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">أحدث المنتجات في السوق</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {loading ? (
          <div className="col-span-4 text-center py-8 text-gray-500 dark:text-gray-400">جاري التحميل...</div>
        ) : suggestedProducts.length === 0 ? (
          <div className="col-span-4 text-center py-8 text-gray-500 dark:text-gray-400">لا توجد منتجات حالياً في السوق.</div>
        ) : (
          suggestedProducts.map((product) => (
            <div 
              key={product.id} 
              onClick={() => navigate('/merchant/marketplace')}
              className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700 group cursor-pointer hover:shadow-md transition-shadow"
            >
              <div className="h-40 bg-gray-50 dark:bg-gray-900 relative flex items-center justify-center p-2">
                {product.images && product.images.length > 0 ? (
                  <img src={product.images[0]} alt={product.title} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 text-sm">لا صورة</div>
                )}
                <div className="absolute top-2 right-2 bg-white dark:bg-gray-800 px-2 py-1 rounded-lg text-xs font-bold text-[#4f46e5]">
                  جملة فقط
                </div>
              </div>
              <div className="p-4 flex flex-col justify-between" style={{ height: 'calc(100% - 10rem)' }}>
                <div>
                  <h4 className="font-bold text-gray-800 dark:text-gray-100 line-clamp-1">{product.title}</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-1">{product.supplier?.company_name}</p>
                </div>
                <div className="mt-4 flex items-center justify-between">
                  <span className="font-bold text-[#4f46e5]">{formatCurrency(product.price)}</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">MOQ: {product.moq}</span>
                </div>
              </div>
            </div>
          ))
