...
export default function NewsTicker() {
  ...
  return (
    <div className="w-full relative z-20 sm:px-6 lg:px-0">
      <div className="w-full bg-white dark:bg-gray-800 sm:rounded-3xl p-3 sm:p-6 shadow-sm border-y sm:border-y-0 border-gray-200 dark:border-gray-700">
        <div className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100 rounded-full py-1.5 overflow-hidden flex items-center relative" style={{ backgroundColor: '#f9fafb' }}>
        ...
        <div className="flex animate-marquee whitespace-nowrap mr-[140px] items-center">
          {displayItems.map((item, index) => (
            <span key={index} className="mx-6 font-bold text-sm flex items-center text-gray-700 dark:text-gray-200">
          ...
          {displayItems.map((item, index) => (
            <span key={`dup-${index}`} className="mx-6 font-bold text-sm flex items-center text-gray-700 dark:text-gray-200">
              <span className="w-1.5 h-1.5 bg-red-500 ml-4 inline-block opacity-40"></span>
              {item}
            </span>
          ))}
